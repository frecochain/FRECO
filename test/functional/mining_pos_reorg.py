...
class ReorgStakeTest(PivxTestFramework):
    ...
    def check_money_supply(self, expected_piv, expected_zpiv):
        ...
        # verify that nodes have the expected FRECO and zFRECO supply
        for node in g_info:
            assert_equal(node['moneysupply'], DecimalAmt(expected_piv))
            for denom in node['zFRECOsupply']:
                assert_equal(node['zFRECOsupply'][denom], DecimalAmt(expected_zpiv[denom]))
    ...
    def run_test(self):
        ...
        # Check FRECO and zFRECO supply at the beginning
        # ------------------------------------------
        # zFRECO supply: 2 coins for each denomination
        ...
        # FRECO supply: block rewards minus burned fees for minting
        ...
        # Verify that FRECO and zFRECO supplies were properly updated after the spends and reorgs
        self.log.info("Check FRECO and zFRECO supply...")

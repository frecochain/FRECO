...
const char * const PIVX_CONF_FILENAME = "freco.conf";
const char * const PIVX_PID_FILENAME = "freco.pid";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\freco
// Windows >= Vista: C:\Users\Username\AppData\Roaming\freco
// Mac: ~/Library/Application Support/freco
// Unix: ~/.freco
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Freco";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\frecoParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\frecoParams
    // Mac: ~/Library/Application Support/frecoParams
    // Unix: ~/.freco-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "FrecoParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "FrecoParams";
#else
    // Unix
    return pathRet / ".freco-params";
#endif
#endif
}
    ...
    std::map<std::string, std::vector<std::string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()) {
        // Create empty freco.conf if it does not exist
        FILE* configFile = fsbridge::fopen(GetConfigFile(), "a");
        if (configFile != NULL){
            std::string strHeader = "# Freco config file\n"
                          "rpcuser=username\n"
    ...
    for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
        // Don't overwrite existing settings so command line settings override freco.conf

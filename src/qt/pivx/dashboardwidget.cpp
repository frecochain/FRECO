    ...
    ui(new Ui::DashboardWidget)
{
    ...
    // Staking Information
    ui->labelMessage->setText(tr("Amount of FRECO earned via Staking & Masternodes"));
	...
	ui->labelAmountMNRewards->setText("0 FRECO");
...
void DashboardWidget::loadWalletModel()
{
 ...
 ///       connect(ui->btnHowTo, &QPushButton::clicked, window);
    ...
    // update the display unit, to not use the default ("FRECO")
    updateDisplayUnit();
}
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair FRECO, zFRECO
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(255, 191, 50));
	set1->setColor(QColor(254, 79, 18));

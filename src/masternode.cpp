...
{
    if (nHeight > 12902401)	return 0.1 * COIN;
    if (nHeight > 6451200)	return 0.5 * COIN;
    if (nHeight > 3225600)	return 1 * COIN;
    if (nHeight > 1612800)	return 2 * COIN;
    if (nHeight > 806400)	return 3 * COIN;
    if (nHeight > 201600)	return 5 * COIN;
    if (nHeight > 36000)	return 4 * COIN;
    if (nHeight > 28800)	return 3 * COIN;
    if (nHeight > 21600)	return 2 * COIN;
    if (nHeight > 14400)	return 1.5 * COIN;
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    // should be at least not earlier than block when 1000 FRECO tx got MASTERNODE_MIN_CONFIRMATIONS
    uint256 hashBlock = UINT256_ZERO;
    CTransaction tx2;
    GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
    BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 FRECO tx -> 1 confirmation

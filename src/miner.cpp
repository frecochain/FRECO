...
{
    // double check that there are no double spent zFRECO spends in this block or tx
    if (tx.HasZerocoinSpendInputs()) {
        ...
        //This zFRECO serial has already been included in the block, do not add this tx.
        if (fDoubleSerial) {
            return false;
        }
    }
...

    {
        ...
        for (CTxMemPool::indexed_transaction_set::iterator mi = mempool.mapTx.begin();
            ...
            for (const CTxIn& txin : tx.vin) {
                ...
                // zFRECO spends can have very large priority, use non-overflowing safe functions
        ...
        while (!vecPriority.empty()) {
            ...
            // zFRECO check to not include duplicated serials in the same block.
